import os
from connection.dataNodeConn import upload_file_with_grpc as upload_grpc
...
def split_file(file_path):
    ...
    with open(file_path, 'rb') as file:
        ...
        while block:
            block_file_name = f"{destination_directory}/{base_file_name}-_-part{block_num:04d}"
...
def join_files(file_name):
    ...
    print(f"File {file_name} joined, check the downloads folder")
...
def send_files_to_datanode(data_node_address, file_name):
    ...
    for partition in partitions:
        ...
        with open(partition_path, 'rb') as partition_file:
            ...
            upload_grpc(partition, content, data_node_address)
